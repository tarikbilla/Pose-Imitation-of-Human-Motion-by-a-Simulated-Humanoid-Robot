...
\normalsize Supervisors: M.Sc.\ Severin Stahl \quad Prof.\ Dr.-Ing.\ Hartmut Weber}
...
Two processes joined by a datagram socket make up the system (Fig.~\ref{fig:arch}). On one
side, a Python pipeline acts as a generic pose source, handling capture, inference, filtering
and gait statistics. On the other, a Webots controller owns everything robot-specific: joint
axes, sign conventions, mechanical limits, balance and gait.

Two considerations fixed this boundary: Webots embeds its own Python interpreter, into which a
CUDA-enabled TensorFlow build is difficult to install reliably; and driving the robot's full
pose requires its actual limb geometry rather than pre-computed angles, so the kinematics
belongs with the robot. All robot-side mathematics therefore resides in
\texttt{main/libraries/} and imports no simulator module, making it unit-testable without
Webots.
...
\begin{tikzpicture}[node distance=4.5mm]
  ...
  \node[blk, right=of cam, minimum width=18mm] (cap) {Capture $+$\\adaptive FPS\\\scriptsize 25 to 100\,Hz};
  ...
  \node[blk, right=of pose, minimum width=18mm] (filt) {speed-adaptive\\\scriptsize low-pass filter};
...
MeTRAbs estimates each frame independently, so the landmark stream must be filtered. A
fixed-$\alpha$ exponential average cannot do it without cost, since its group delay of
$(1-\alpha)/\alpha$ samples is independent of subject motion: at $14.3$~FPS, $\alpha = 0.50$
costs $70$~ms of the latency budget on the keypoint channel alone, directly observable as robot
lag. The pipeline instead applies a speed-adaptive low-pass
filter~\cite{casiez2012oneeuro} per axis per landmark, whose cutoff is a function of measured
speed, so the delay is spent only while the subject is still. With a minimum cutoff of
$1.0$~Hz and $\beta = 0.005$~Hz per mm/s, a limb moving at $1000$ to $2000$~mm/s opens the
cutoff by $5$ to $10$~Hz; with Nyquist at $7.2$~Hz, that range is the difference between
smoothing and pass-through.
...
\begin{tikzpicture}[node distance=3.2mm, every node/.style={font=\scriptsize}]
  \node[blk, text width=48mm] (dbl)
       {\textbf{DOUBLE}: symmetric crouch,\\the validated no-fall baseline};
  \node[acc, below=of dbl, text width=48mm] (load)
       {\textbf{LOAD}: lean until the centre\\of mass is over the stance foot};
  \node[blk, below=of load, text width=48mm] (gate)
       {stance margin $>0$~?\\forward kinematics with link\\masses, plus foot force sensors};
  \node[acc, below=of gate, text width=48mm] (sgl)
       {\textbf{SINGLE}: swing leg follows\\the human, authority from that margin};
  \draw[flow] (dbl) -- node[lbl, right=1pt] {foot raised} (load);
  \draw[flow] (load) -- (gate);
  \draw[flow] (gate) -- node[lbl, right=1pt] {pass} (sgl);
  \draw[ret] (sgl.east) -- ++(4mm,0)
       |- node[lbl, accline, pos=0.25, right=1pt] {UNLOAD} (dbl.east);
\end{tikzpicture}
...
is lowered, the margin is lost, or the torso tilts past $0.40$~rad.}
\label{fig:liftfsm}
\end{figure}
...
\textbf{Preparation ramp.} Every clip opens in a sole-flat crouch of about $0.51$~rad while
the controller stands at $u = 0.10$, and playback commands its first keyframe immediately with
the velocity caps lifted, so direct handover would demand $0.84$~rad of knee travel in one
$20$~ms step. The arbiter therefore ramps the legs to the clip's opening pose and starts
playback only once the measured joints arrive. They must arrive \emph{together}: the knee
travels $0.842$~rad, the hip $0.405$ and the ankle $0.437$, and sole attitude is their sum, so
a single flat rate drives that sum to $0.405$~rad, eight times the tilt budget, and the
support margin to $-0.064$~m. Scaling each joint's rate by its share of the longest travel
holds the sum at $0.0000$~rad and the margin at $+0.0596$~m in the same $0.56$~s. The return is
a ramp too, seeded from the posture the clip leaves.

\textbf{Exit policy.} A clip boundary is a balanced double-support pose, so playing the whole
clip is the safe default. The cost is that clip length becomes the latency of stopping, and
that every stride pays the start-stop transient. That transient is $49\%$ of \texttt{Forwards.motion}'s
$2.60$~s, and the closing settle carries the torso $17$~mm backward, which is why chained clips
measured $0.036$~m/s against a documented $0.10$~m/s. Three exits exist, in order of
preference: leaving a detectable gait cycle, stopping at a certified keyframe, and the tilt
abort. The second carries a momentum term that a purely static test misses, since stopping the
legs does not stop the robot: the capture point~\cite{pratt2006capture} lies $v/\omega$ ahead
of the centre of mass with $\omega \approx 5.9$~rad/s at this crouch, so $0.18$~m/s throws it
$30$~mm past, against a $40$ to $60$~mm budget. Thirteen of the $46$ statically acceptable
poses are moving that fast.

\textbf{Cyclic gait.} Each clip is an animation comprising a squat, an acceleration, some
strides, a deceleration and a stand. Within it, the stride itself is satisfactory; what costs
is the transient around it. Inspection shows the middle of the long walk clip to be a true
limit cycle, with
...
giving $0.073$~m/s sustained. The schedule is derived from the keyframes at run time rather than tabulated, so it cannot
diverge from the shipped files, and a non-cyclic clip is detected rather than looped on
assumption. Two conditions keep this honest: the seam must be exact to one microradian, being a
teleport executed in one step with the caps lifted, and the cycle must translate the robot by
at least $0.02$~m, since looping a turn or side-step would rotate or drift it indefinitely.
Cycling does not lengthen the stride; it removes the transient between strides, reducing a
$10$~s walk from 47 stand-squat cycles to one.
...
stance-foot odometry already used for translation, yields a ladder of $104$ angles spaced at
most $16\degree$ apart. The controller re-evaluates the live heading error each step and stops at the
...
reporting completion freezes the robot. A watchdog bounds suspension and retires the offending
clip; a backoff abandons clips after three bad attempts, on the reasoning that repeated falling
is worse than never walking; and a settled-start check admits clips only from an upright, calm
robot. Section~9 reports a session in which that backoff proved too blunt. Separately, an
exception in a control step is logged and the body reclaimed rather than terminating the loop,
which previously zeroed every motor velocity.
...
Failures in this project are statistical, such as a head pinned for $40\%$ of
...
\begin{subfigure}[t]{0.325\textwidth}
  ...
  \caption{Arms extended laterally; torso yaw $+4.0\degree$.}
  \label{fig:shot-tpose}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.325\textwidth}
  \includegraphics[width=\linewidth]{imgs/2.png}
  \caption{Single-arm raise; torso yaw $+18.2\degree$.}
  \label{fig:shot-raise}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.325\textwidth}
  \includegraphics[width=\linewidth]{imgs/3.png}
  \caption{Torso yaw $+81.2\degree$; the robot has stepped round.}
  \label{fig:shot-turn}
\end{subfigure}
\caption{Live operation: the Webots simulation (left of each pane) beside the annotated camera
feed (right). The overlay reports throughput, per-frame pipeline latency, landmark count and
the current gait, yaw and action verdicts. Across these captures the pipeline held $17.5$ to
$18.7$~FPS at $53.4$ to $57.1$~ms with all $25$ landmarks tracked, while the heading servo
followed the subject's torso yaw from $+4.0\degree$ to $+81.2\degree$.}
\label{fig:shots}
\end{figure*}
...
$\approx 56$~ms. Over the $18{,}437$ frames of the session in Fig.~\ref{fig:shots} the whole
perception loop closes in $56.1$~ms at the median and $87.4$~ms at the 90th percentile, i.e.\
$17.8$~FPS sustained, agreeing with the on-screen overlay; an earlier $31{,}243$-frame session
gave $63$~ms and $16.0$~FPS. Inference is therefore essentially the entire budget:
the gait cue costs $0.030$~ms, the action cue $0.011$~ms, the landmark filter $0.022$~ms per
...
Summing independently measured stages gives $\approx 115$~ms from camera exposure to motor
command for the arms and head: one $56$~ms frame period, up to one $20$~ms control step of
quantisation, and $38$~ms of residual filter lag, the last replacing a frame-rate-clocked
exponential filter that cost $108$~ms. This meets the $150$~ms requirement at the median but
not at the 90th percentile. It remains a sum of parts rather than one stopwatch reading,
because the controller logs its own step counter rather than the pipeline's frame index; that
single field is the most valuable instrumentation still missing.
...
further $2.5$~hours, and was still in progress when this analysis was taken. Throughout it,
...
The two cues are better treated as independent witnesses to a single question: either the
pelvis travels, or the legs cycle. Letting the verdicts that a cycling pair of legs contradicts
yield to a confident march raised coverage of marching intervals by clip-driven walking from $12.5\%$ to
...
Scope is bounded first by what the robot may attempt. Aerial and highly dynamic motion such
as flips, jumps and running is excluded by construction: every leg command passes a quasi-static
centre-of-mass test that presumes continuous ground contact, and the simultaneous absence of
both feet is treated as a tracking failure rather than a manoeuvre. Backward locomotion is
weaker than forward: \texttt{Backwards.motion} contains no detectable limit cycle, so unlike
the forward clip it cannot be rewound and must be replayed one shot at a time, paying its
start-stop transient on every step and bounding a backward walk at roughly half the forward
sustained speed. Lateral motion shares that limitation. Two defects found in the session logs
are also open. The locomotion backoff retires all clips after three failures
...
End-to-end latency remains a sum of measured stages rather than one measurement, because the
controller logs its own step counter instead of the pipeline's frame index. Two items warrant a
check on the target machine: the torso-yaw sign convention against real MeTRAbs output, and
whether the IMU pitch scale can be raised toward the analytically correct $2.0$. The
accelerometer path wants a complementary or Kalman filter fusing gyro integration with gravity.
Finally, the action classifier's walk witnesses are absolute speeds fitted to one subject;
expressing them relative to leg length and a noise floor learned at session start would remove
the recalibration the two-witness trigger currently compensates for. A per-joint error study
against fixed reference motions would turn the imitation-fidelity requirement from a target
into a measurement; the logs and tooling exist.
...
pass at full fidelity while displacing motions receive a bounded compensating pelvis shift.
The project's defects share a character worth recording: the playback-confirmation defect, the
double-clamped pelvis shift, the axis-masking artefact, the force-sensor veto, the
non-convergent overshoot fraction and the locomotion kill switch were all silent, the robot
continuing to behave plausibly while a stated invariant was violated. None was found by
observation; all were found by stating the invariant the code claimed and measuring, across
whole recorded sessions, whether it held.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Acknowledgments}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

We thank M.Sc.~Severin Stahl and Prof.~Dr.-Ing.~Hartmut Weber (THM, Campus Friedberg, IEM)
for supervising this project and for guidance on scope, requirements and evaluation
methodology. This work was carried out as Group~C's project for the CSPM 2026S course. The
