...
clip is the safe default --- but that makes clip length the latency of stopping and pays the
...
\textbf{Cyclic gait.} The clips are animations --- squat, acceleration, stride, deceleration,
stand --- in which the stride is satisfactory and the surrounding transient is the cost. The
middle of the long walk clip is a true limit cycle, with
...
Scope is bounded first by what the robot may attempt. Aerial and highly dynamic motion ---
flips, jumps, running --- is excluded by construction: every leg command passes a quasi-static
...
The authors thank M.Sc.~Severin Stahl and Prof.~Dr.-Ing.~Hartmut Weber (THM, Campus
Friedberg, IEM) for supervising this project and for guidance on scope, requirements and
evaluation methodology. The work was carried out as Group~C's project for the CSPM 2026S
course. The authors declare no conflicts of interest.
