...
measured at $0.0000$~rad, raising sustained walking speed from $0.036$ to $0.089$~m/s. The
implementation is $21{,}794$ lines of Python, of which $7{,}433$ are tests. We report measured
perception timings, stability outcomes from instrumented runs, and a Webots
...
travel-proportional ramp and continuous by a measured limit-cycle rewind; and the diagnosis
of a Webots \texttt{InertialUnit} defect that halves the reported torso pitch.
...
$\max_j |q_j(2.84\,\mathrm{s}) - q_j(1.80\,\mathrm{s})| = 0.0000$~rad over all twelve leg
joints, and periodicity holding to that precision from $1.80$ to $5.32$~s. The segment can
therefore be rewound with \texttt{setTime()} without commanding joint motion at the seam,
making one clip a gait generator: a $1.04$~s stride advancing $0.093$~m, giving $0.089$~m/s
sustained. The schedule is derived from the clip's keyframes at run time rather than tabulated,
...
On an RTX~3090~Ti, per-frame inference costs $75.9$~ms with full detection and $36.2$~ms with a
tracked box, giving $16.0$~FPS end to end at a detection interval of 2. The 1\euro{} filter
removed $70$~ms of constant group delay from the keypoint channel, and retargeting, smoothing
and UDP transmission each cost below $1$~ms per frame. These are component measurements: full
camera-to-actuation latency against the $150$~ms requirement has not been measured on the
target machine and is not estimated here. Table~\ref{tab:results} collects the balance and
locomotion outcomes.

\begin{table}[H]
\centering
\caption{Measured outcomes from instrumented simulation runs.}
\label{tab:results}
\small
\begin{tabular}{@{}p{4.0cm}p{3.1cm}@{}}
\toprule
Quantity & Measured \\
\midrule
Longest continuous stand, IMU attitude & $298$\,s \\
Stand with accelerometer attitude & fell in $\approx 2$\,s, $6/6$ \\
Sustained walk speed, cyclic clip & $0.089$\,m/s \\
Walk speed, chained clips & $0.036$\,m/s \\
Limit-cycle seam error, 12 joints & $0.0000$\,rad \\
Preparation ramp, sole attitude & $0.0000$\,rad (flat: $0.405$) \\
Preparation ramp, support margin & $+0.0596$\,m (flat: $-0.064$) \\
Planner in standing state & $83$--$86\%$ of decisions \\
\bottomrule
\end{tabular}
\end{table}

The implementation is $21{,}794$ lines of Python: $7{,}433$ of tests, $5{,}705$ of robot-side
mathematics, $3{,}258$ of Webots controllers, $3{,}219$ of pipeline and $2{,}179$ of tooling.
Because every robot-side library is simulator-independent, the control stack is testable
without a Webots installation. The suite covers the leg solve as a round trip,
self-calibration and cropped-feet lift detection, the step sequence and every safety gate,
clip discovery and the overshoot guard, the centre-of-mass model and its spiral search, and a
...
including leg arbitration, motion hand-off and freeze resistance.

At the time of writing \texttt{pytest} collects 419 tests, of which 417 pass, one is skipped
and one fails. The failure concerns the cyclic-clip exit phase of Section~6: with a synthetic
cyclic clip the controller remains in its cycling state and does not reach the leaving state
within the asserted window. It is recorded as a known open item and should be resolved before
the cyclic-gait path is relied upon for an assessed demonstration.
...
recovering it requires hand landmarks \texttt{coco\_19} does not provide. Several results were
derived analytically or from recorded logs rather than from a live GPU session, and three items
warrant a first-run check on the target machine: the torso-yaw sign convention against real
MeTRAbs output, end-to-end latency against the $150$~ms requirement, and whether the IMU pitch
scale can be raised toward the analytically correct $2.0$. Locomotion remains clip-bounded,
with stride length and turn granularity fixed by the shipped clips; an online gait generator or
a learned policy~\cite{ze2025twist} would remove that quantisation, and the heading servo
already accepts a continuous action. The accelerometer path wants a complementary or Kalman
filter fusing gyro integration with gravity. Finally, a per-joint error study against fixed
reference motions would convert the imitation-fidelity requirement from a target into a
measurement; the logs and tooling for it already exist.
