...
VENV_PYTHON = os.path.join("C:\\", "venvs", "a3-pose", "Scripts",
                           "python.exe")
UNLIMITED = 86400.0
...
def main():
    ...
    parser.add_argument("--seconds", type=float, default=0.0,
                        help="Session limit in seconds, 0 = unlimited")
    parser.add_argument("--python", default=None,
                        help="Interpreter for the driver (default: the a3 venv)")
    ...
    interpreter = args.python or (VENV_PYTHON if os.path.isfile(VENV_PYTHON)
                                  else sys.executable)
    if not os.path.isfile(interpreter):
        print(f"interpreter not found: {interpreter}")
        return 1

    limit = args.seconds if args.seconds > 0 else UNLIMITED
    env = dict(os.environ)
    env.update({"A3_LIVE": "1", "A3_WALL_CLOCK": "1", "A3_WAIT_DRIVER": "0",
                "A3_RUN_SECONDS": str(limit)})
    ...
    command = [interpreter, "-u", os.path.join(TOOLS, "live_puppet.py")]
    if args.site:
    ...
    print(f"driver    {interpreter}")
    print(f"session   {'unlimited' if args.seconds <= 0 else str(args.seconds) + ' s'}")
    print("starting driver - press q in the monitoring window to end "
          "the session\n")
    try:
        finished = subprocess.run(command, cwd=A3_ROOT, env=env)
        if finished.returncode:
            print(f"\ndriver exited with code {finished.returncode}. "
                  "If this was an ImportError, the interpreter above is "
                  "not the a3 venv; pass --python explicitly.")
